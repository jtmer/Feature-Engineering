...
def load_timesfm_model(
    ...
    model = timesfm.TimesFm(
        ...
        checkpoint=timesfm.TimesFmCheckpoint(
            huggingface_repo_id=model_name,  # "google/timesfm-2.0-500m-pytorch"
        ),
    )
    return model


class TimesFMICLTrainer(ICLTrainer):
    def __init__(
        self,
        model: "TimesFmTorch",
        n_features: int,
    ...
    def predict_long_horizon(
        ...
        for dim in range(self.n_features):
            ...
            tensor_lists = [
                [sublist[0] for sublist in batch] for batch in tensor_ts.tolist()
            ]

            predictions, _ = self.model.forecast(
                inputs=tensor_lists, freq=[0] * len(tensor_lists)
            )
            predictions = (
