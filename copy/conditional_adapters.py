...
class NeuralLatentEncoder(nn.Module):
    ...
    def __init__(
        ...
        # x branch: (B,Cx,L) -> (B, z_dim, L)
        self.covariates_encoder = build_temporal_encoder(
            input_channels=covariates_dim_aug,
            output_channels=latent_dim,
            num_layers=encoder_layers,
        )
        self.covariates_gate = nn.Sequential(
            nn.Conv1d(covariates_dim_aug, hidden_dim, kernel_size=1),
            nn.GELU(),
            nn.Conv1d(hidden_dim, latent_dim, kernel_size=1),
            nn.Sigmoid(),
        )
        self.latent_fuse = nn.Conv1d(2 * latent_dim, latent_dim, kernel_size=1)
    ...
    def forward(self, target_series: torch.Tensor, covariates_series: torch.Tensor):
        ...
        latent_from_target = self.target_encoder(target_series)                 # (B,Cz,L)
        latent_from_covariates = self.covariates_encoder(covariates_series)     # (B,Cz,L)
        latent_gate = self.covariates_gate(covariates_series)                   # (B,Cz,L)

        latent_series = torch.cat(
            [latent_from_target, latent_from_target + latent_gate * latent_from_covariates],
            dim=1,
        )
        latent_series = self.latent_fuse(latent_series)                         # (B,Cz,L)
...
class NeuralTargetDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.covariate_residual_scale = 0.4
        ...
        # regression trunk: (B, z_dim, L) -> (B, 1, L)
        self.latent_decoder = build_temporal_decoder(
            input_channels=latent_dim,
            output_channels=1,
            num_layers=decoder_layers,
        )

        # covariate residual branch: (B, Cx, L) -> residual / gate
        self.covariates_proj = nn.Sequential(
            nn.Conv1d(covariates_dim_aug, hidden_dim, kernel_size=1),
            nn.GELU(),
