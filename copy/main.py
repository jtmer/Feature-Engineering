...
def build_parser():
    ...
    p.add_argument("--past_pretrain_patience", type=int, default=10)

    p.add_argument("--lambda_latent_scale", type=float, default=1e-4)
    p.add_argument("--latent_rms_target", type=float, default=12.0)
    ...
    p.add_argument("--proxy_cov_ratio_floor", type=float, default=0.40)
    ...
    p.add_argument("--lambda_latent_stats", type=float, default=0.1)
    ...
    p.add_argument("--earlystop_patience", type=int, default=8)
...
def main():
    ...
    #     (2025, 6),
    # ]
    # test_months = [datetime(y, m, 1) for y, m in selected_months]
    ...
    for m0 in test_months:
        ...
        X_tr_all_raw, y_tr_all_raw, _ = build_xy_from_df(d_train_all, target_col=args.target_col, drop_cols=drop_cols)
        scaler_X_all, scaler_y_all = fit_scalers(X_tr_all_raw, y_tr_all_raw)
        X_tr_all_s, y_tr_all_s = scale_xy(X_tr_all_raw, y_tr_all_raw, scaler_X_all, scaler_y_all)

        y_p_tr_all, x_p_tr_all, x_f_tr_all, y_f_tr_all = make_windows(
        ...
        logger.info(
            f"[final_refit windows] train_all={y_p_tr_all.shape[0]}  "
            f"test(daily)={y_p_te.shape[0]}"
        )

        if y_p_tr_all.shape[0] == 0:
            logger.info(f"[skip] Final refit train_all windows empty in month {month_tag}")
        else:
            if model2.adapter.has_stats_predictor:
                logger.info(">>> Final refit future_stats_predictor on train_all ...")
                model2.pretrain_stats_predictor(
                    future_target=y_f_tr_all,
                    future_covariates=x_f_tr_all,
                    n_epochs=cfg.stats_pretrain_epochs // 4,
                    batch_size=cfg.stats_pretrain_bs,
                    lr=cfg.stats_pretrain_lr // 5,
                    weight_decay=cfg.stats_pretrain_wd,
                    patience=cfg.stats_pretrain_patience,
                    val_data=None,
                    verbose=True,
                ...
                logger.info(">>> Done final refit stats predictor.")
            else:
                logger.info(">>> Skip final refit stats_pretrain (pipeline has no stats_predictor).")

            if model2.adapter.can_reconstruct_past:
                logger.info(">>> Final refit past reconstruction only ...")
                model2.pretrain_past_reconstruction_only(
                    past_target=y_p_tr_all,
                    past_covariates=x_p_tr_all,
                    val_data=None,
                    n_epochs=cfg.past_pretrain_epochs // 4,
                    min_n_epochs=cfg.past_pretrain_min_epochs // 4,
                    batch_size=cfg.past_pretrain_bs,
                    lr=cfg.past_pretrain_lr // 5,
                    weight_decay=cfg.past_pretrain_wd,
                    patience=cfg.past_pretrain_patience,
                    lambda_proxy_floor=cfg.lambda_proxy_floor,
                    proxy_sens_floor=cfg.proxy_sens_floor,
                    ...
                    lambda_latent_std_floor=cfg.lambda_latent_std_floor,
                    debug=True,
                    debug_dir=str(out_root / f"debug_no_revin_residual_refit_{month_tag}"),
                    # debug_plot=True,
                    debug_plot=False,
                    verbose=True,
                ...
                logger.info(">>> Done final refit past reconstruction.")
            else:
                logger.info(">>> Skip final refit past_reconstruction_pretrain (pipeline encoder/decoder is no-op).")

            if model2.adapter.is_trainable:
                logger.info(">>> Final refit adapter on train_all ...")
                model2.train_adapter(
                    past_target=y_p_tr_all,
                    past_covariates=x_p_tr_all,
                    future_target=y_f_tr_all,
                    future_covariates=x_f_tr_all,
                    n_epochs=cfg.train_epochs // 4,
                    batch_size=cfg.train_bs,
                    lr=cfg.train_lr // 5,
                    weight_decay=cfg.train_wd,
                    lambda_past_recon=cfg.lambda_past_recon,
                    ...
                    use_swanlab=False,
                    debug=args.debug,
                    debug_dir=str(out_root / f"debug_train_adapter_refit_{month_tag}"),
                    debug_every=args.debug_every,
                    debug_num_seq=args.debug_num_seq,
                    debug_latent_ch=args.debug_latent_ch,
                    earlystop_patience=args.earlystop_patience,
                ...
                logger.info(">>> Done final refit adapter.")
            else:
                logger.info(">>> Skip final refit train_adapter (pipeline is not trainable).")
